...
/**
 * Precompiled io.firefly.java-conventions script plugin.
 **/
public class IoFireflyJavaConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        ...
        target.getPluginManager().apply("net.linguica.maven-settings");
        target.getPluginManager().apply("com.github.spotbugs");
        target.getPluginManager().apply("project-report");
        target.getPluginManager().apply("com.avast.gradle.docker-compose");
        target.getPluginManager().apply("maven-publish");
        target.getPluginManager().apply("nebula.dependency-recommender");
        target.getPluginManager().apply("io.firefly.jacoco-convention");
        target.getPluginManager().apply("info.solidsoft.pitest");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_IoFireflyJavaConventions").asSubclass(BasicScript.class);
