...
/**
 * Precompiled io.firefly.aws-lambda-conventions script plugin.
 **/
public class IoFireflyAwsLambdaConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("io.firefly.java-conventions");
        ...
        target.getPluginManager().apply("org.springframework.boot.experimental.thin-launcher");
        target.getPluginManager().apply("com.github.johnrengelman.shadow");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_IoFireflyAwsLambdaConventions").asSubclass(BasicScript.class);
