...
/**
 * Precompiled io.firefly.dependency-check script plugin.
 **/
public class IoFireflyDependencyCheckPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("org.owasp.dependencycheck");
        target.getPluginManager().apply("com.github.ben-manes.versions");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_IoFireflyDependencyCheck").asSubclass(BasicScript.class);
