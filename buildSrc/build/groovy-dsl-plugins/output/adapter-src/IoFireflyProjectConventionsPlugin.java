...
/**
 * Precompiled io.firefly.project-conventions script plugin.
 **/
public class IoFireflyProjectConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        ...
        target.getPluginManager().apply("io.firefly.quality-convention");
        target.getPluginManager().apply("io.firefly.dependency-check");
        target.getPluginManager().apply("maven-publish");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_IoFireflyProjectConventions").asSubclass(BasicScript.class);
